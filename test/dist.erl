...
start()->
    ...
    % Start NodeLog
    os:cmd("rm -rf test_logs"),
    ok=file:make_dir("test_logs"),
    LogFile=filename:join(["test_logs","db_log"]),
    ...
    {ok,Nodes,NodeNames,NodeHostNameList}=setup(),
    
    ok=application:set_env([{db,[{db_callbacks,?DbCallBacks},
				 {nodes,[]}]}]),
    ok=application:start(db),
    io:format("2. Start local ~p~n",[{?MODULE,?FUNCTION_NAME,?LINE}]),
    
    ok=check_all([node()]),

    % 2. Install First Node
    [{db_application_spec,ok},
     {db_deployment_info,ok},
     {db_deployments,ok},
     {db_host_spec,ok}]=db_oam:install_first_node(),
    FirstNode=db_oam:first_node(),
    ok=check_all([FirstNode]),
    ...
    %% 2. Install OaM to start the databases 
    io:format("2. Intial install ~p~n",[{?MODULE,?FUNCTION_NAME,?LINE}]),
    
    EtdcStart=[{Node,rpc:call(Node,appl_start,[[]])}||Node<-Nodes],
...
setup()->
   ...
   ok=test_nodes:start_nodes(),
    ...
    %% set db_xx in test_ebin and set path 
    [rpc:call(Node,code,add_patha,["test_ebin"])||Node<-Nodes],

    {ok,HostName}=net:gethostname(),
    NodeNames=test_nodes:get_nodenames(),
    ["c100","c200","c201","c202","c300"]=NodeNames,
    NodeHostNameList=[{list_to_atom(NodeName++"@"++HostName),NodeName}||NodeName<-NodeNames],
    {ok,Nodes,NodeNames,NodeHostNameList}.
